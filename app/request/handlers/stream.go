...
func (h Handler) HandleXadd(args []string) resp.Data {
	...
	s, err := h.store.Xadd(key, stream_key, key_vals)
	if err != nil {
...
func (h Handler) HandleXrange(args []string) resp.Data {
	if len(args) != 3 {
	...
	entries, err := h.store.XRange(key, startStr, endStr)
	if err != nil {
		return resp.Err(err.Error())
	}
...
func (h Handler) HandleXread(args []string) resp.Data {
